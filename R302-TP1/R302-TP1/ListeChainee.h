...
bool ListeChainee<TypeInfo>::insereAtPositIter(const int nouvellePosition, const TypeInfo &nouvelleInfo) {
    ...
    if (nouvellePosition >= 1 && nouvellePosition <= this->nbCellules+1){
        if (nouvellePosition == 1){
            insereTeteWorker(this->ptrTete,nouvelleInfo);
        } else {
            Cellule<TypeInfo> *currentCellule = this->ptrTete;
            for (int i = 1; i < nouvellePosition - 1; i++) {
                currentCellule = currentCellule->getSuivante();
            }
            Cellule<TypeInfo> *CelluleToInsert = new Cellule<TypeInfo>(nouvelleInfo,currentCellule->getSuivante());
            currentCellule->setSuivante(CelluleToInsert);
            nbCellules++;
        }
        return true;
    } else{
...
bool ListeChainee<TypeInfo>::supprimeAtPositIter(int position) {
    ...
    if (position >= 1 && position <= this->nbCellules+1){
        if (position == 1){
            supprimeTeteWorker(this->ptrTete);
        } else {
            int positionPrec = 0;
            Cellule<TypeInfo> *currentCellule = this->ptrTete;
            ...
            Cellule<TypeInfo> *celluleASupprimer = currentCellule->getSuivante();
            currentCellule->setSuivante(celluleASupprimer->getSuivante());
            celluleASupprimer->setSuivante(nullptr);
