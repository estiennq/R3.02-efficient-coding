...
int ArbreNoeudBinaireRecherche<TypeInfo>::getHauteurRecWorker(NoeudBinaire<TypeInfo>* ptrRac) const {
    if (ptrRac == nullptr){
        ...
        return 1
        + max (getHauteurRecWorker(ptrRac->getRefPtrFilsGauche()),getHauteurRecWorker(ptrRac->getRefPtrFilsDroit()));
    }
...
TypeInfo ArbreNoeudBinaireRecherche<TypeInfo>::getAncetreCommunLePlusBasRecWorker(const NoeudBinaire<TypeInfo>* ptrRac,
    ...
    if (ptrRac->getInfo() < val1 && ptrRac->getInfo() > val2 || ptrRac->getInfo() < val2 && ptrRac->getInfo() > val1 ){
...
//    if (monPtrRac->getPtrFilsDroit() != sonPtrRac->getPtrFilsDroit() || monPtrRac->getPtrFilsGauche() != sonPtrRac->getPtrFilsGauche()){
//        return false;
//    } else {
...
//    }
    if (monPtrRac == nullptr && sonPtrRac == nullptr){
        ...
        return aMemeGeometrieQueRecWorker(monPtrRac->getPtrFilsGauche(), sonPtrRac->getPtrFilsGauche()) && aMemeGeometrieQueRecWorker(monPtrRac->getPtrFilsDroit(), sonPtrRac->getPtrFilsDroit());
    } else {
        return false;
    }
